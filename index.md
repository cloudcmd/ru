...
Если не задано параметров Cloud Commander читает информацию с `json/config.json` и использует стандартный порт (`8000`), если переменных с именами `cloud9`, `cloudfoundry` и `nodester` не существует.
